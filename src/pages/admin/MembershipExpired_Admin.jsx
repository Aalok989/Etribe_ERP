...
const getPlanId = (planName) => {
  return plans.find(plan => plan.plan_name === planName)?.id;
};
...
export default function MembershipExpired() {
  ...
  const { refreshMembers } = useDashboard();
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (paymentResponse && (paymentResponse.status === true || paymentResponse.success === true || paymentResponse.data)) {
        toast.success('Membership renewed and payment recorded successfully!');
        setMembers(prevMembers => prevMembers.filter(member => member.id !== modifyMember.id));
        navigate('/members-services/payment-details');
        closeModify();
        refreshMembers(); // Refresh members in the dashboard context
        
        // Also refresh the local expired members list
        fetchExpiredMembers();
      } else {
        ...
        // Still refresh members data even if payment failed, since membership was activated
        refreshMembers();
      }
    } catch (err) {
      if (err.response) {
        const errorMessage = err.response.data?.message || err.response.data?.error || 'Failed to renew membership or record payment';
        setUpdateError(
        ...
        setUpdateError('Failed to renew membership or record payment. Please try again.');
        toast.error('Failed to renew membership or record payment.');
      }
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
              <div className="bg-white dark:bg-[#202123] rounded-xl shadow-xl dark:shadow-2xl p-6 w-full max-w-md relative max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Renew Membership</h2>
                  <button
                ...
                <form className="space-y-4" onSubmit={e => e.preventDefault()}>
                  ...
                  {updateError && (
                    <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
                      <div className="flex items-center gap-2">
                        <FiAlertCircle />
                        <span>{typeof updateError === 'string' ? updateError : updateError?.message ? updateError.message : JSON.stringify(updateError)}</span>
                      </div>
                    </div>
                  )}
