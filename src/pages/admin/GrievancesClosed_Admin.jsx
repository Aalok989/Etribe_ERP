...
export default function GrievancesClosed() {
  ...
  const fetchGrievances = async () => {
    try {
      ...
      if (err.response?.status === 401) {
        ...
        toast.error("Closed grievances endpoint not found. Please check the API configuration.");
        setGrievances([]);
      } else {
        toast.error(err.response?.data?.message || err.message || "Failed to fetch closed grievances");
        setGrievances([]);
      }
    } finally {
      setLoading(false);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Closed Grievances");
    XLSX.writeFile(wb, "closed_grievances.xlsx");
    toast.success("Closed grievances exported to Excel!");
  };
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", "closed_grievances.csv");
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast.success("Closed grievances exported to CSV!");
  };
  ...
  const copyToClipboard = () => {
    ...
    navigator.clipboard.writeText(text).then(() => {
      toast.success("Closed grievances copied to clipboard!");
    }).catch(() => {
  ...
  const exportToPDF = () => {
    ...
    try {
      ...
      doc.text("Closed Grievances Report", 40, 40);
      ...
      doc.setFont("helvetica", "normal");
      doc.text(`Total Closed Grievances: ${filteredGrievances.length}`, 40, summaryY + 15);
      ...
      <DashboardLayout>
        <div className="min-h-screen flex items-center justify-center bg-white dark:bg-[#1E1E1E]">
          <div className="flex items-center gap-3">
            ...
            <p className="text-indigo-700 dark:text-indigo-300">Loading closed grievances...</p>
          </div>
        </div>
      </DashboardLayout>
    );
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-6 py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-red-600">Closed Grievances</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-1">Manage and track all closed grievance complaints</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
              <FiAlertTriangle className="text-red-600" />
              <span>Total Closed: {grievances.length}</span>
            </div>
            <button 
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 flex-1">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  className="pl-10 pr-4 py-3 border rounded-lg text-sm bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 focus:ring-2 focus:ring-red-400 transition-colors w-full"
                  value={searchTerm}
        ...
        {currentEntries.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-400">
              {searchTerm ? 'No grievances match your search criteria.' : 'No closed grievances at the moment.'}
            </p>
          </div>
        )}
          ...
          <div className="bg-white dark:bg-[#1E1E1E] rounded-xl shadow-lg p-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                ...
                <select
                  className="border rounded-lg px-3 py-1 text-sm bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100 text-gray-700 focus:ring-2 focus:ring-red-400 transition-colors"
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`p-2 rounded-lg text-red-600 dark:text-red-300 hover:bg-red-100 dark:hover:bg-gray-700 transition-colors ${
                      ...
                      <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                        ...
                        <div className="flex items-center gap-2">
                          <select
                            ...
                            className="text-xs border border-gray-300 dark:border-gray-600 rounded px-2 py-1 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-red-500 focus:border-red-500 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                          ...
                          {isUpdatingStatus && (
                            <FiRefreshCw className="animate-spin text-red-600 text-xs" />
                          )}
                        </div>
                      </div>
