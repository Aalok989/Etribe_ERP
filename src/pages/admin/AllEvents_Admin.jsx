...
export default function AllEvents() {
  ...
  const validateForm = () => {
    const errors = {};
    if (!addEventForm.event.trim())
      errors.event = "The Event Title field is required.";
    if (
      !addEventForm.agenda ||
      !addEventForm.agenda.replace(/<[^>]*>/g, "").trim()
    )
      errors.agenda = "The Agenda field is required.";
    if (!addEventForm.venue.trim())
  ...
  const validateEditForm = () => {
    const errors = {};
    if (!editEventForm.event.trim())
      errors.event = "The Event Title field is required.";
    if (
      !editEventForm.agenda ||
      !editEventForm.agenda.replace(/<[^>]*>/g, "").trim()
    )
      errors.agenda = "The Agenda field is required.";
    if (!editEventForm.venue.trim())
      errors.venue = "The Venue field is required.";
      ...
      <div className="flex flex-col gap-4 py-3">
                    ...
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                      Agenda <span className="text-red-500">*</span>
                       ...
                       className={`rounded-lg p-1 bg-white dark:bg-gray-700 ${
                         formErrors.agenda ? "border border-red-500" : ""
                       }`}
                     >
                      <RichTextEditor
        ...
        <div className="rounded-2xl shadow-lg bg-white dark:bg-[#1E1E1E] w-full">
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {paginated.map((event, idx) => (
                  <tr key={event.id || idx} className={`border-b border-gray-200 dark:border-gray-700 transition-colors ${idx % 2 === 0 ? 'bg-white dark:bg-[#1E1E1E]' : 'bg-gray-50 dark:bg-[#202123]/50'} hover:bg-indigo-50 dark:hover:bg-gray-700 hover:shadow-sm`}>
          ...
          <div className="lg:hidden space-y-4 p-4">
            {paginated.map((event, idx) => (
              <div key={event.id || idx} className="bg-white dark:bg-[#1E1E1E] rounded-lg border border-gray-200 dark:border-gray-700 p-4 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      value={addEventForm.event}
                      onChange={handleAddEventChange}
                      className={`w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-400 dark:focus:ring-orange-300 focus:border-transparent transition-colors ${
                        formErrors.event
                          ? "border-red-500"
                          : "border-gray-200 dark:border-gray-600"
                      }`}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div
                      className={`rounded-lg p-1 bg-white dark:bg-gray-700 dark:text-gray-100 ${
                        formErrors.agenda ? "border border-red-500" : ""
                      }`}
                    >
                      <RichTextEditor
