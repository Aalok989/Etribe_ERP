...
const getMemberTableHeaders = (additionalFields = []) => {
  ...
  return [...baseHeaders, ...middleHeaders, ...endHeaders];
};
...
export default function PTFIMembers() {
  ...
  // Get active members from dashboard context
  const dashboardMembers = dashboardData?.members?.active || [];
  ...
  // Sort data
  const sortedData = [...dashboardMembers].sort((a, b) => {
  ...
  const filtered = sortedData.filter(member => 
    ...
    (member.address || member.location || member.ad2)?.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleExportCopy = () => {
    ...
    navigator.clipboard.writeText(data);
          toast.success("All PTFI members copied to clipboard!");
  };
  ...
  const handleExportExcel = () => {
    ...
    try {
      const exportData = dashboardMembers.map(member => ({
        ...
        Email: member.email || '',
        Address: member.address || member.location || member.ad2 || '',
  ...
  const handleExportCSV = () => {
    ...
    const rows = dashboardMembers.map(member => [
      member.name,
      member.phone_num || member.contact,
      member.email,
      member.address || member.location || member.ad2 || '',
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute("download", "ptfi_members.csv");
    ...
    toast.success("PTFI Members exported to CSV!");
  };
              ...
              <tbody>
                {paginated.map((member, idx) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-left border-r border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-100">{member.address || member.location || member.ad2 || 'N/A'}</td>
              ...
              <div key={member.id} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-700 dark:text-gray-300 truncate">{member.address || member.location || member.ad2 || 'N/A'}</span>
                  </div>
