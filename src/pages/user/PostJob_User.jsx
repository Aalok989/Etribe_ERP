...
export default function PostJobUserPage() {
  ...
  const userCompany = useMemo(() => {
    return activeMembers.find(
      (member) =>
        String(member.user_detail_id || member.user_id || member.id || member.company_detail_id) === uid
    );
  }, [activeMembers, uid]);

  const userCompanyId = userCompany
    ? String(userCompany.company_detail_id || userCompany.id || userCompany.user_id || "")
    : "";
  const userCompanyName =
    userCompany?.company_name ||
    userCompany?.name ||
    userCompany?.company ||
    (userCompanyId ? `Company #${userCompanyId}` : "Your Company");
  ...
  useEffect(() => {
    if (userCompanyId) {
      setAddJobForm((prev) => ({
        ...prev,
        company_detail_id: userCompanyId,
      }));
    }
  }, [userCompanyId]);
  ...
  const handleAddJobSubmit = async (e) => {
    ...
    if (!userCompanyId) {
      setFormError("Unable to determine your company. Please contact support.");
      return;
    }

    if (!addJobForm.job_type) {
                ...
                <div className="flex-1 overflow-y-auto space-y-3 sm:space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      readOnly
                      className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 cursor-not-allowed"
                    />
                    <input type="hidden" name="company_detail_id" value={userCompanyId} />
                  </div>
